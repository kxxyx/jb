...
var path = "/sdcard/script/youtube.js";
updates();
...
var win_1 = floaty.rawWindow(
    <frame id="logo" w="44" h="44" alpha="0.4" >//悬浮按钮
        ...
        <img id="img_logo" w="32" h="32" src="http://47.111.31.150:8000/data/autojs_material.png" gravity="center" layout_gravity="center" />
...
var win_2 = floaty.rawWindow(
    <frame id="logo" w="{{device.width}}px" h="44" alpha="0" >//悬浮按钮 弹性替身
        <img w="44" h="44" src="#ffffff" circle="true" alpha="0.8" />
        <img id="img_logo" w="32" h="32" src="http://47.111.31.150:8000/data/autojs_material.png" margin="6 6" />
    </frame>
)
...
function downjs() {
    toastLog("需要更新");
    var xzfh = http.get("http://47.111.31.150:8000/lua/youtube.js");
    if (xzfh.statusCode != 200) {
        toast("请求失败");
    } else {
        files.ensureDir("/sdcard/script/");
        files.writeBytes("/sdcard/script/youtube.js", xzfh.body.bytes());
        toastLog("更新成功");
        return "t"
    }
}
