...
function addfriend(){
	...
	var tag = "[添加朋友]";
    var conturl="http://"+__SERVER+"/index.php?g=api&m=sj&a=jzwphone&id="+bhid+"&uid="+uid;
	...
	for (i  = 0; i < __CONT.length; i++) {
        ...
        if(currentActivity() == "com.tencent.mm.plugin.profile.ui.ContactInfoUI" && text('添加到通讯录').exists()){
            ...
            http.get("http://"+__SERVER+"/index.php?g=api&m=sj&a=postphonezt&zt=1&id=" +__CONT[i]["number"]);
            sleep(600);
...
function mmlauncherui(){
    ...
    sleep(2000);
    if(text('微信').exists()&&text('选择打开方式').exists()){
        click("微信");  
        sleep(2000);
    }
    var lc=0;
...
function tjphonezt(){
    ...
    var conturl="http://"+__SERVER+"/index.php?g=api&m=sj&a=allphone&id="+bhid+"&uid="+uid+"&snum=1";
