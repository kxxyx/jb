...
function openclosewang2(){

    while(1){
        var sz = getPackageName("设置");
        if (sz){
            launch(sz); 
        }
        sleep(2000);

        if(text('其他连接方式').exists()&&text('蓝牙').exists()){
            click("其他连接方式");
            sleep(2000);
            click("飞行模式");
            sleep(2000);
        ...
        toastLog("简介");
        scrollDown(2);
        if(descContains('介绍').exists()){
            descContains("介绍").findOne().click();
            sleep(5000);
            classNameContains("EditText").findOne().setText(signature);
            sleep(2000);
            click("保存");
            sleep(5000);
        }
        ...
        //爱好
        if(descContains('添加').exists()){
            descContains("添加").findOne().click();
            sleep(5000);
            if(text('爱好').exists()){
                classNameContains("CheckBox").click();
                sleep(2000);
                click("保存");
                sleep(5000);
            }
