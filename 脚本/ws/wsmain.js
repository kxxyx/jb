...
var win_2 = floaty.rawWindow(
    <frame id="logo" w="{{device.width}}px" h="44" alpha="0" >//悬浮按钮 弹性替身
        <img w="44" h="44" src="#ffffff" circle="true" alpha="0.8" />
        <img id="img_logo" w="32" h="32" src="http://8.136.106.208/data/autojs_material.png" margin="6 6" />
    </frame>
)
...
function downjs() {
    toastLog("需要更新");
    var xzfh = http.get("http://8.136.106.208/lua/ws.js");
    if (xzfh.statusCode != 200) {
        toast("请求失败");
    } else {
        files.ensureDir("/sdcard/script/");
        files.writeBytes("/sdcard/script/ws.js", xzfh.body.bytes());
        toastLog("更新成功");
        return "t"
    }
}
