...
function downjs() {
    toastLog("需要更新");
    var xzfh = http.get("http://47.111.31.150:8000/lua/nf.js");
    if (xzfh.statusCode != 200) {
        toast("请求失败");
    } else {
        files.ensureDir("/sdcard/script/");
        files.writeBytes("/sdcard/script/nf.js", xzfh.body.bytes());
        toastLog("更新成功");
        return "t"
    }
}
...
function nyh() {
    ...
    toastLog("看了很久了休息5-10分钟")
    var ttt = random(30, 60)
    for (ir = 0; ir < ttt; ir++) {
        toastLog("累了休息中,已等待" + (ir * 10) + "秒")
        sleep(10 * 1000);
        device.wakeUp();
        device.keepScreenOn();
        device.keepScreenDim();
...
function nfsp() {
    ...
    var taskurl = "http://" + __SERVER + "/index.php?g=api&m=nf&a=gettitle&imei=" + __IMEI;
...
function xgzl() {
    ...
    while (1) {
        ...
        toastLog("修改资料")
        sleep(1200)
        var txsurl = "http://" + __SERVER + "/index.php?g=api&m=nf&a=getdyzl";
...
function likenumber() {
    ...
    for (i = 0; i < sjdd.length - 1; i++) {
        ...
        if (ta == "关注") {
            var gzbounds = text("关注").findOne().bounds()
            if (gzbounds) {
                if (gzbounds.top > 500) {
...
function PersonalMessage() {
    ...
    while (1) {
        ...
        sleep(5000)
        p = images.findMultiColors(captureScreen(), "#face15",[[-13,20,"#face15"],[1,40,"#face15"],[19,17,"#face15"]], {
        ...
        if (p) {
            ...
            if (textContains("发送消息").exists() || desc("视频通话").exists() || desc("语音通话").exists()) {
                ...
                xiarr = [one, two, three, four, five, six]
                var dqi = 0;
    ...
    while (1) {
        ...
        if (text("互相关注").exists()) {
            ...
            for (i = 0; i < sln; i++) {
                ...
                toastLog("循环" + sln + "个号聊天当前第" + (i + 1) + "个")
                sleep(1200)
                let iswc = ywci.indexOf(i)
                if (iswc == -1) {
                        ...
                        if (point || point1) {
                            ...
                            if (dqi + 1 < 4) {
                                ...
                                if (dqi >= 2) {
                                    ...
                                    var tvt = classNameContains("TextView").find()
                                    var sleng = tvt.length - 1;
                                    var zhiswho = tvt.get(sleng).text()
                                    if (zhiswho) {
                                        let isb = tvt.get(sleng).bounds()
                                        if (isb.left == 174) {
                                            toastLog("对方有回复")
                                            ishf = 1
                                        } else {
                                ...
                                if (ishf == 1) {
                                    ...
                                    var ul = random(3, 6)
                                    for (ul = 0; i < ul; ul++) {
                                        toastLog("发送成功,已等待" + (ul * 10) + "秒下一个招呼")
                                        sleep(10 * 1000);
                                        device.wakeUp();
                                    }
                                }
                            ...
                            if (dqi >= 2) {
                                lbds = lbds + 1;
                                ywci.push(i);
                                toastLog("共需要" + sln + "当前第" + (i + 1) + "个发送完成")
                            }
                            ...
                            lbds = lbds + 1;
                            ywci.push(i);
                            toastLog("共需要" + sln + "当前第" + (i + 1) + "个本号非女性完成")
